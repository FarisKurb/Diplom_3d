"""Dijkstra pathfinding strategy.

Wraps the existing visibility-graph + Dijkstra pipeline into the
:class:`PathfindingStrategy` interface so it can be used interchangeably
...
class DijkstraStrategy(PathfindingStrategy):
    """Standard Dijkstra shortest-path on a visibility graph.
    ...
    def _get_samples(self, mesh: Mesh) -> List[Vector3]:
        """Lazily compute and cache samples keyed by mesh identity."""
        key = id(mesh)
